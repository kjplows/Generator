...
int main(int argc, char ** argv)
{
  ...
  int Z_init = (gOptProbePDG > 0) ? 0 : 1; // configure with neutron for neutrinos, proton for anti
  InitialState free_init(Z_init, 1, gOptProbePDG);
  //intgen_map->BuildMap(free_init); // the initial state is a bit of a dummy
  intgen_map->BuildMap(InitialState(18, 40, gOptProbePDG)); // the initial state is a bit of a dummy
  ...
  for( ; evgliter != evglist->end() ; ++evgliter ) {
    ...
    */
    ...
    for( ; intliter != ilst.end(); ++intliter ) {
      ...
      // Set the probe momentum
      // RETHERE more numbers
      double nuE = 1.0; // GeV
      TLorentzVector nup4(0., 0., nuE, nuE);
	...
	if( iy != iy_prev ) {
	  LOG("gqelmap", pNOTICE) << "iy --> " << iy << " , Eb = " << 1000. * Eb << " MeV" ;
	  iy_prev = iy;
	}
